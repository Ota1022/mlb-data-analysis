...
header = ["順位", "選手名","age","debut_year","tall","weight","pro_year","position","bt","Date","Home Tm","Away Tm","PA","AB","R","H","2B","3B","HR","RBI","BB","SO","SB","CS","HBP","AVG","OBP","SLG","OPS"]
...
# Create a BeautifulSoup object to parse the HTML content
    ...
    for row in rows[1:26]:
        ...
        player_url = 'https://baseballsavant.mlb.com/savant-player'+'/'+str.lower('-'.join(player_name.split(' ')))+'-'+row.find('a')['href'].split('/')[2]+'?stats=gamelogs-r-hitting-mlb&season='+str(season) 
        url_year='https://www.mlb.com/player/'+str.lower('-'.join(player_name.split(' ')))+'-'+row.find('a')['href'].split('/')[2]+'?stats=gamelogs-r-hitting-mlb&year=2019'
        response_year = requests.get(url_year)
        soup_year = BeautifulSoup(response_year.content, 'html.parser')
        
        if soup_year.find('div',{'class':'player-bio'}):

            tables_year = soup_year.find('div',{'class':'player-bio'}).find('ul')
            debut_year=tables_year.text.split("\n")[-2].split(':')[-1]
        else:
            debut_year = 0
        ...
        tall =float(table_new[5].replace("'",""))*30.48 + float(table_new[6].replace('"',""))*2.54
        age = table_new[10]
